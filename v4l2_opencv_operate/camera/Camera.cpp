...
//如果需要保存yuyv或y数据流，定义以下宏即可
//#define SAVE_YUYV
//#define SAVE_Y_COMPONENT
...
//	fmt.fmt.pix.pixelformat	= V4L2_PIX_FMT_YUV420;V4L2_PIX_FMT_GREY
...
//    CaptureReleaseFrame(index);
	...
	// 直接保存的yuyv数据
//	char filename[50];
//	sprintf(filename,"yuv%d.yuv",i);
	FILE *file = fopen("yuv0.yuv", "wb");
	fwrite((char*)(char *) buffers[index].start, 1, buffers[index].length, file);
	fclose(file);
#endif

	memcpy( buffer, (char *) buffers[index].start,  buffers[index].length);
#if 0
	*data_len = 0;
...
#endif
	*data_len = buffers[index].length;
	if( *data_len != buffers[index].length)
		printf_err( "datalen: %d, buffers[index].length: %d\n", *data_len, buffers[index].length );
...
int Camera::get_y_component(unsigned char *buffer, unsigned char *y_buffer,int * data_len)
{
	if(!buffer || !y_buffer || !data_len)
		return -EINVAL;

	*data_len = 0;
	char *imp = (char *)buffer;
	int z = 0;
	for (unsigned int y = 0; y < height; y++)
	{
		unsigned int offset = y * width * 2;
...
//			printf_dbg("x:%d y:%d offset %d\n", x, y, offset);
			y_buffer[ off + x] = imp[offset + z];
			(*data_len)++;
			z+=2;
		}
	}
#ifdef SAVE_Y_COMPONENT
	FILE *file = fopen("yuv-y.yuv", "wb");
	fwrite((char*)y_buffer, 1, height*width, file);
	fclose(file);
#endif
...
int Camera::set_REGISTER(int location,int value)
{
    int ret;
	...
	ret = ioctl(fd, VIDIOC_DBG_S_REGISTER, &ctrl);
	if(ret < 0) {
		printf_err("errno %d \n", errno);
		printf_err("set REGISTERR failed, %s\n", strerror(errno));
		return ret;
	}else{
                printf_dbg("set REGISTERR OK\n");
        }
	return 1;
}

int Camera::get_REGISTER(int location,__u64 *value)
{
	int ret;
	struct v4l2_dbg_register ctrl;
	memset(&ctrl, 0, sizeof(ctrl));
        ctrl.reg=location;
	//printf_warn("ctrl.val= %x \n",(unsigned char)(ctrl.val&0xff));
	ret = ioctl(fd, VIDIOC_DBG_G_REGISTER, &ctrl);
	if(ret < 0) {
		printf_err("read errno %d\n", errno);
		printf_err("get 0x0a REGISTER failed, %s\n", strerror(errno));
		return ret;
	}
	*value = ctrl.val;

        printf_dbg("ctrl.val= %x \n",(unsigned char)(ctrl.val&0xff));
	return 1;
}
